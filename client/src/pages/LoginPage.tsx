		...
		<div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px', background: '#000' }}>
			<motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={{ width: '100%', maxWidth: 440 }}>
				...
				<div style={{ textAlign: 'center', marginBottom: 40 }}>
					...
					<p style={{ color: 'var(--gold)', fontSize: '0.6rem', letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: 12 }}>Welcome Back</p>
					<h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2.8rem', fontWeight: 400, color: '#f5f5f0', margin: 0 }}>Sign In</h2>
				</div>
				...
				<div style={{ background: '#0a0a0a', border: '1px solid #1e1e1e', padding: '36px' }}>
					...
					<form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
						...
						<div style={{ position: 'relative' }}>
							<label style={{ display: 'block', color: 'var(--white-muted)', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8 }}>Email</label>
							<div style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(30%)', color: focused === 'email' ? 'var(--gold)' : '#333', transition: 'color 0.2s' }}>
								<Mail size={15} />
							</div>
							<input type='email' required value={email} onChange={e => setEmail(e.target.value)}
								onFocus={() => setFocused('email')} onBlur={() => setFocused(null)}
								style={{ ...inputStyle, borderColor: focused === 'email' ? 'var(--gold)' : '#1e1e1e' }}
						...
						{/* Password */}
						<div style={{ position: 'relative' }}>
							<label style={{ display: 'block', color: 'var(--white-muted)', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8 }}>Password</label>
							<div style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(30%)', color: focused === 'pass' ? 'var(--gold)' : '#333', transition: 'color 0.2s' }}>
								<Lock size={15} />
							</div>
							<input type='password' required value={password} onChange={e => setPassword(e.target.value)}
								onFocus={() => setFocused('pass')} onBlur={() => setFocused(null)}
								style={{ ...inputStyle, borderColor: focused === 'pass' ? 'var(--gold)' : '#1e1e1e' }}
								placeholder='••••••••'
							/>
						</div>
						...
						>
							{loading ? <><Loader size={15} style={{ animation: 'spin 1s linear infinite' }} />Signing in...</> : <><LogIn size={15} />Sign In</>}
						</button>
					</form>
					...
					<p style={{ marginTop: 24, textAlign: 'center', color: 'var(--white-muted)', fontSize: '0.8rem' }}>
						...
						<Link to='/signup' style={{ color: 'var(--gold)', textDecoration: 'none', fontWeight: 500 }}>
							Create one <ArrowRight style={{ display: 'inline', verticalAlign: 'middle' }} size={13} />
						</Link>
					</p>
				</div>
			</motion.div>
